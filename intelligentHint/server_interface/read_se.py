...
#Read the SE file from input file
def read_se_string(input_string):
...
#Converts the SE file to CSV file (differential), at this point in time
...
# List all the operations that exist in completeSE, and save them into operations.csv
def calculate(dir):
    all_operation = set()
...
# Create success objects by shuffling some blocks in the compleseSE file
...
# Create failure objects by removing some blocks in the compleseSE file
def generate_mock_se_failure(input_file, n):
    """
    Create new .se file with some blocks deleted

    Args:
        input_file : file_name excluding directory
        n : identifying number for output filename
    """
    with open("completeSE/" + input_file, encoding='utf-8') as f:
        content = f.readlines()
...
def read_operation(opt_file):
    ...
    # `Unnamed: 0` contains all key-value information
